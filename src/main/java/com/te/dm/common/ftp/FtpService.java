package com.te.dm.common.ftp;

import com.te.dm.bean.Ftp;
...
public class FtpService {
     ...
     * 构造函数.
     */
    public FtpService(Ftp ftp) {
        this.ftp = ftp;
        this.ftpClient = new FTPClient();
        //如果不调用这个方法设置端口，则是使用默认端口21
        this.ftpClient.setDefaultPort(ftp.getPort());
    }
    ...
    public void openConnect() throws IOException {
        ...
        int reply; // 服务器响应值
        // 连接至服务器
        ftpClient.connect(this.ftp.getHostName());
        // 获取响应值
        ...
        // 登录到服务器
        if (this.ftp.getUserName() != null && !this.ftp.getUserName().equals("")) {
            ftpClient.login(this.ftp.getUserName(), this.ftp.getPassword());
        } else {
            //无用户名登录时
